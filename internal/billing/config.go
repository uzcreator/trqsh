...
type Config struct {
	SecretKey     string // RIFT_STRIPE_SECRET_KEY (sk_test_… / sk_live_…)
	WebhookSecret string // RIFT_STRIPE_WEBHOOK_SECRET (whsec_…)
	...
	MeterBandwidth string // RIFT_STRIPE_METER_BANDWIDTH
	MeterRequests  string // RIFT_STRIPE_METER_REQUESTS

	SuccessURL      string // RIFT_BILLING_SUCCESS_URL
	CancelURL       string // RIFT_BILLING_CANCEL_URL
	PortalReturnURL string // RIFT_BILLING_PORTAL_RETURN_URL

	TrialDays        int           // RIFT_BILLING_TRIAL_DAYS (Pro trial)
	DunningGrace     time.Duration // RIFT_BILLING_DUNNING_GRACE (past_due -> Free)
	WebhookTolerance time.Duration // signature timestamp tolerance
	MeteringInterval time.Duration // RIFT_BILLING_METERING_INTERVAL (0 = off)
}
...
func LoadConfig() Config {
	c := DefaultConfig()
	env(&c.SecretKey, "RIFT_STRIPE_SECRET_KEY")
	env(&c.WebhookSecret, "RIFT_STRIPE_WEBHOOK_SECRET")
	env(&c.MeterBandwidth, "RIFT_STRIPE_METER_BANDWIDTH")
	env(&c.MeterRequests, "RIFT_STRIPE_METER_REQUESTS")
	env(&c.SuccessURL, "RIFT_BILLING_SUCCESS_URL")
	env(&c.CancelURL, "RIFT_BILLING_CANCEL_URL")
	env(&c.PortalReturnURL, "RIFT_BILLING_PORTAL_RETURN_URL")

	c.Prices = map[string]StripePrices{
		PlanPro: {
			Monthly: os.Getenv("RIFT_STRIPE_PRICE_PRO_MONTHLY"),
			Annual:  os.Getenv("RIFT_STRIPE_PRICE_PRO_ANNUAL"),
		},
		PlanTeam: {
			Monthly: os.Getenv("RIFT_STRIPE_PRICE_TEAM_MONTHLY"),
			Annual:  os.Getenv("RIFT_STRIPE_PRICE_TEAM_ANNUAL"),
		},
		PlanPAYG: {
			Metered: os.Getenv("RIFT_STRIPE_PRICE_PAYG_METERED"),
		},
	}
	if v := os.Getenv("RIFT_BILLING_TRIAL_DAYS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			c.TrialDays = n
		}
	}
	if v := os.Getenv("RIFT_BILLING_DUNNING_GRACE"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			c.DunningGrace = d
		}
	}
	if v := os.Getenv("RIFT_BILLING_METERING_INTERVAL"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
