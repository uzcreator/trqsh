...
//
// The rift module is consumed via a relative replace so the GUI embeds the very
// same agent core (internal/agent) the CLI uses — no drift.
module github.com/rift/rift/gui

go 1.25.0

require (
	github.com/rift/rift v0.0.0
	github.com/wailsapp/wails/v3 v3.0.0-alpha.11
)

replace github.com/rift/rift => ../
